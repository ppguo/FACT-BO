...
class TabPFNRegressor(RegressorMixin, BaseEstimator):
    ...
    _OUTPUT_TYPES_BASIC = ("mean", "median", "mode", "ei", "pi", "variance", "ucb")
...
def _logits_to_output(
    *,
    ...
    elif output_type == "ucb":
        output = criterion.ucb(logits=logits, best_f=best_f, rest_prob=0.05), # noqa: ERA001
    else:
        raise ValueError(f"Invalid output type: {output_type}")
